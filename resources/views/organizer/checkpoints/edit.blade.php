<x-app-layout title="Edit Checkpoint – Jejak Hijau" :user="auth()->user()">
    <div class="px-4 py-6 max-w-3xl mx-auto space-y-6">
        ...
        {{-- Form --}}
        <form id="edit-checkpoint-form" method="POST" action="{{ route('organizer.checkpoints.update', $checkpoint->id) }}" class="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm space-y-6" novalidate>
            @csrf
            @method('PUT')
